  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className="relative p-2 rounded-xl text-slate-200 hover:text-white hover:bg-white/10 transition-colors focus:outline-none"
      >
        <Bell size={20} />
        {unreadCount > 0 && (
          <span className="absolute 1.5 -top-0.5 -right-0.5 bg-gov-saffron-500 text-white text-[10px] font-bold h-4.5 min-w-4.5 px-1 rounded-full flex items-center justify-center ring-2 ring-gov-navy-950 animate-pulse">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden text-gov-navy-900 animate-in fade-in slide-in-from-top-2 duration-150">
          {/* Header */}
          <div className="p-3.5 px-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-gov-navy-900">Notifications</span>
              {unreadCount > 0 && (
            ...
            {unreadCount > 0 && (
              <button 
                onClick={markAllRead} 
                className="text-xs text-gov-saffron-700 hover:text-gov-saffron-800 font-medium flex items-center gap-1"
              >
                <Check size={13} />
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-slate-100 custom-scrollbar">
            {notifications.length === 0 ? (
              <div className="py-8 px-4 text-center">
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">We'll alert you about scheme matches and application updates.</p>
              </div>
            ) : (
          ...
          <div className="p-2.5 bg-slate-50 border-t border-slate-100 text-center">
            <Link 
              ...
              className="text-xs font-semibold text-gov-navy-800 hover:text-gov-saffron-700 flex items-center justify-center gap-1 transition-colors"
            >
